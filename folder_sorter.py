...
# -*- coding: utf-8 -*-
"""
Folder Sorter - organizes episode and movie files into folders.
...
Part of Media Toolbox; can also be run on its own:  python folder_sorter.py
"""
...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QPushButton, QTableView, QTableWidget, QTableWidgetItem, QTabWidget,
    QVBoxLayout, QWidget,
)

from common import (
    ERROR_COLOR, OK_COLOR, DIM_COLOR, FolderPicker, HistoryDialog, HistoryStore, ToolTab,
    _is_checked, legacy_config_path, load_tool_settings, move_files, remember_recent,
    revert_batch, run_standalone, sanitize_component, save_tool_settings, unique_path,
)

# =============================================================== settings

SETTINGS_KEY = "folder_sorter"
...
BUILTIN_DETECTORS = {
    ...
    # Spelled-out episode markers. "Cap"/"Capitulo" cover Spanish releases.
    "episode": {
        "label": "Title Episode 02 / Ep 02 / Cap 02",
        "example": "Mushishi Episode 03.mkv",
        "regex": rf"(?i)^{LEADING_GROUP}(?P<name>.+?)[\s_.\-]+(?:episode|ep|cap[ií]tulo|cap)[\s_.]*\d{{1,4}}(?!\d)",
        "target": "stem",
        "clean": True,
    },
}
...
DETECTOR_ORDER = ["original", "dash", "sxxexx", "nxnn", "episode", "underscore"]
...
def load_settings() -> dict:
    # Imports settings.json from the standalone Folder Sorter the first time.
    s = load_tool_settings(SETTINGS_KEY, DEFAULT_SETTINGS,
                           legacy=legacy_config_path("FolderSorter", "settings.json"))
    ...
    # Built-in detectors added in a later version show up at the end, turned off,
    # so an update never changes how existing files are sorted.
    known = {d.get("id") for d in s["detectors"]}
    s["detectors"] = list(s["detectors"]) + [
        {"id": i, "enabled": False} for i in DETECTOR_ORDER if i not in known]
    return s
...
def folder_key(name: str) -> str:
    """Comparison key that ignores case, spaces and punctuation, used to merge
    'Bounen no Xamdou', 'bounen_no_xamdou' and 'Bounen no Xamdou!' into one folder."""
...
def execute_moves(source_root: Path, output_root: Path, jobs: list[tuple[str, str]],
    ...
    jobs: [(filename, relative destination)].
    Returns (batch for the history file or None, {filename: (status, new path or None)}).
    """
    abs_jobs = [(source_root / f, output_root / dest / f) for f, dest in jobs]
    ...
    batch, results = move_files(abs_jobs, on_exists, label="Folder Sorter",
                                root=str(source_root), output=str(output_root),
                                progress=progress, cancelled=cancelled)
    # Results are keyed by full source path; the tab works with file names.
    return batch, {Path(k).name: v for k, v in results.items()}
...
COLS = ["", "File", "Destination", "Reason", "Status"]
C_INC, C_FILE, C_DEST, C_REASON, C_STATUS = range(5)
...
class PlanModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == R.DisplayRole:
            return [None, it.filename, it.destination or "-", it.reason, it.status][c]
...
class FolderSorterTab(ToolTab):
    title = "Folder Sorter"

    def __init__(self, parent=None):
        super().__init__(parent)
        self.settings = load_settings()
        self.history = HistoryStore(SETTINGS_KEY, legacy=legacy_config_path("FolderSorter", "history.json"))
        ...
        for b in (b_all, b_none, b_rules, b_hist):
            bottom.addWidget(b)
        bottom.addStretch()
    ...
    def scan(self):
        ...
        if self.scanned_root == root:
            manual = {i.filename: i for i in self.model.items if i.manual and i.status == "Pending"}

        # Only files directly in the folder: subfolders are what we already organized.
        exts = tuple(e.lower() for e in self.settings.get("extensions", [".mkv"]))
        try:
            files = sorted(e.name for e in os.scandir(root) if e.is_file() and e.name.lower().endswith(exts))
        except OSError as e:
            QMessageBox.warning(self, "Can't read folder", str(e))
            return

        items = []
        for f in files:
            if f in manual:
                items.append(manual[f])
                continue
            dest, reason = decide(f, self.settings)
            items.append(PlanItem(f, dest, reason, include=bool(dest), current_path=root / f))
        ...
        if self.settings.get("merge_similar_folders", True):
            out = self.output_root()
            existing = []
            if out and out.is_dir():
                try:
                    existing = sorted(e.name for e in os.scandir(out) if e.is_dir())
                except OSError:
                    pass
    ...
    def organize(self):
        ...
        if out.exists() and not out.is_dir():
            QMessageBox.warning(self, "Output folder", f"'{out}' is a file, not a folder.")
            return
    ...
    def show_menu(self, pos):
        ...
        a.setEnabled(bool(first.destination) and out is not None and (out / first.destination).is_dir())
        m.addSeparator()
    ...
    def change_destination(self, items: list[PlanItem]):
        ...
        if out and out.is_dir():
            try:
                choices |= {e.name for e in os.scandir(out) if e.is_dir()}
            except OSError:
                pass
...
if __name__ == "__main__":
    run_standalone(FolderSorterTab)
